...
-- internal
local iconFileName="plugins/lua-runner.v1.1/run.png"
local mmenuId="mmluarun"     -- mainmenu item
...
function Init()
	...
	local mRun = mainMenu:Find(mmenuId)
	if not mRun then
		mRun = mainMenu:Insert(mmenuIdx,mmenuId)
		mRun._={caption="&Run"}
	end
	-- do not duplicate
	...
	local plug1 = mRun:Insert(menuIdx, pluginInfo.pluginMenuId)
