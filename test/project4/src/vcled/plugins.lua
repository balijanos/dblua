...
local VCL,_VCLED_VERSION,file = VCL,_VCLED_VERSION,file
...
function Load()
	if not plugmanForm then
		...
		plugmanPanel._  = {caption="", align="alBottom", height=30, bevelouter="bvLovered", bevelinner="bvLowered"}
		local ooButton = VCL.SpeedButton(plugmanPanel)
		ooButton._ = {flat=false, caption="On/Off", left=20, top=5, width=100, onclick="Plugins.onSetOnOff"}
		local addButton = VCL.SpeedButton(plugmanPanel)
		addButton._ = {flat=false, caption="Add", left=150, top=5, width=100, onclick="Plugins.onAdd"}
		local removeButton = VCL.SpeedButton(plugmanPanel)
		removeButton._ = {flat=false, caption="Remove", left=280, top=5, width=100, onclick="Plugins.onRemove"}
		local saveButton = VCL.SpeedButton(plugmanPanel)
		saveButton._ = {flat=false, caption="Save", left=410, top=5, width=100, onclick="Plugins.onSave"}
		local cancelButton = VCL.SpeedButton(plugmanPanel)
		cancelButton._ = {flat=false, caption="Close", left=540, top=5, width=100, onclick="Plugins.onCancel"}
	end
...
function AddToPluginList(f)
	...
	local pi = dofile(f)	
	if pi and pi.VCLedversion==_VCLED_VERSION  then
		...
		VCL.ShowMessage(string.format("Plugin version mismatch or invalid plugin information!\nFile: %s",file.getname(f)))
	end
end
